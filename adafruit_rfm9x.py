...
_RH_RF95_REG_2C_RSSI_WIDEBAND = const(0x2C) # For TRNG
...
class RFM9x:
    ...
    _BUFFER = bytearray(4)
    modemconf1 = 0 # to save and restore modem confs
    modemconf2 = 0 # before / afer getting random numbers
    # see: setupLoRandom, resetLoRa, getRandomNumber, stockupRandom at the bottom
    ...
    def setupLoRandom(self):
        """Saves the LoRa modem's original configuration and puts it in Continuous mode"""
        self.modemconf1 = self._read_u8(_RH_RF95_REG_1D_MODEM_CONFIG1)
        self.modemconf2 = self._read_u8(_RH_RF95_REG_1E_MODEM_CONFIG2)
        self._write_u8(_RH_RF95_REG_01_OP_MODE, 0b10001101)
        #MODE_LONG_RANGE_MODE 0b1xxxxxxx || LowFrequencyModeOn 0bxxxx1xxx || MODE_RX_CONTINUOUS 0bxxxxx101
        time.sleep(0.1)
        self._write_u8(_RH_RF95_REG_1D_MODEM_CONFIG1, 0b01110010)
        time.sleep(0.1)
        self._write_u8(_RH_RF95_REG_1E_MODEM_CONFIG2, 0b01110000)
        time.sleep(0.1)
        
    def resetLoRa(self):
        """Puts back the LoRa modem in its original configuration"""
        self._write_u8(_RH_RF95_REG_01_OP_MODE, 0b10000001)
        # MODE_LONG_RANGE_MODE 0b1xxxxxxx || MODE_STDBY 0bxxxxxxx1
        time.sleep(0.1)
        self._write_u8(_RH_RF95_REG_1D_MODEM_CONFIG1, self.modemconf1)
        time.sleep(0.1)
        self._write_u8(_RH_RF95_REG_1E_MODEM_CONFIG2, self.modemconf2)
        time.sleep(0.1)

    def getRandomNumber(self):
        """Computes an 8-bit random number from the RssiWideband Register,
        adding some whitening to aid with randomness."""
        x = 0
        i = 0
        b = self._read_u8(_RH_RF95_REG_2C_RSSI_WIDEBAND) & 0b00000001
        while i<8:
            while (b == self._read_u8(_RH_RF95_REG_2C_RSSI_WIDEBAND) & 0b00000001):
                # Whitening: von Neumann extractor
                #  0, 0 or 1, 1 discard
                #  0, 1 --> 0
                #  1, 0 --> 1
                b = self._read_u8(_RH_RF95_REG_2C_RSSI_WIDEBAND) & 0b00000001
    ...
    def stockupRandom(self, count = 256):
        """Fills up a bytearray with count random bytes"""
        if count == 0:
            return
        finalBuff=bytearray(count)
        ix = 0
        self.setupLoRandom()
        while ix<count:
            finalBuff[ix] = self.getRandomNumber()
            ix += 1
        self.resetLoRa()
        return finalBuff
